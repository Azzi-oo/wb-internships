...
var infoCache = make(map[string]jsonStruct, 10)
...
func getDataById(r *gin.Context) {
	id := r.Param("id")
	result, ok := infoCache[id]
	if ok {
		resultJSON, err := json.MarshalIndent(result, "", "    ")
		if err != nil {
			panic(err)
		}
		r.HTML(http.StatusOK, "report.html", gin.H{
			"report": string(resultJSON),
		})
		return
	}
	r.IndentedJSON(http.StatusNotFound, gin.H{"message": "info not found"})
}
...
func main() {
	...
	fmt.Println("Starts program")
	fmt.Printf("DB Data fetch attempt: %v\n", infoCache)

	op, err := stan.Connect("test-cluster", "client-ildarkhan", stan.NatsURL("nats://localhost:4222"))

	go op.Publish("berry", []byte(jsonData))

	sub, _ := op.Subscribe("berry", func(m *stan.Msg) {
		curData := jsonStruct{}
